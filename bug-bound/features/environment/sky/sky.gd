...
@export_range(0.0, 1.0, 0.01) var day_progress: float = 0.0:
	set(value):
		day_progress = value
		_update_visuals()
@export var moon_direction: Vector3 = Vector3(0.5, 0.5, -0.5)
...
func _ready() -> void:
	if (
		world_environment and 
		world_environment.environment and 
		world_environment.environment.sky
	):
...
func _process(delta: float) -> void:
	...
	if not Engine.is_editor_hint():
		day_progress = ContextSystem.world_context.time
	else:
