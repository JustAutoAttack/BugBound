...
func physics_update(delta: float) -> void:
	...
	if current_substate == SubState.WALK:
		...
		_owner.velocity.x = move_toward(_owner.velocity.x, 0, _owner.walk_speed * delta)
		_owner.velocity.z = move_toward(_owner.velocity.z, 0, _owner.walk_speed * delta)
...
func _get_random_navigation_point() -> Vector3:
	...
	var random_dist: float = randf_range(1.0, _owner.wander_radius)
	var offset: Vector3 = Vector3(cos(random_angle), 0, sin(random_angle)) * random_dist
